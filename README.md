...
每個遊戲畫面都有 **結束遊戲** 與 **回到主選單**（需確認）：

- **結束遊戲**：伺服器強制結束該房目前對局。飛行棋／棋類／猜猜畫畫回到該遊戲候機室；踩地雷回到模式選擇。房間裡其他人一齊離開對局畫面。
...
- **過三關**：3×3，三連即勝。棋盤最多同時 4 子：落下第 5 子時先看有沒有三連，沒有則最舊的一子消失再判一次。格子角落數字是落下順序。
